...
export function Friends() {
  ...
  useEffect(() => {
    ...
    const setupListenersAndFetchData = async () => {
      ...
      // 6. Fetch Initial Friends List & Listen to Status
      try {
  ...
  // ---------------------------
    ...
    const getChatDisplayInfo = (chat: Chat): { name: string; photoURL?: string; status?: string } => {
        ...
        const otherFriend = otherUserId ? friends.find(f => f.id === otherUserId) : null;
  ...
  const handleSendMessage = async (e?: FormEvent) => {
    ...
    try {
      ...
      await sendMessage(
        ...
        if (!currentAttachedFile && !currentAttachedAudio) setNewMessage(currentMessageText);
  ...
  ), [chats, searchQuery, user, friends, onlineFriendIds]);
