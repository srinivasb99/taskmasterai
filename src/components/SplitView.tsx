...
import {
    ...
    Lock,
    Trash2,
    Sparkles, // Removed unused Copy import
    BookOpen,
    X,
    SplitSquareVertical
} from 'lucide-react';
...
export function SplitView({
    ...
    // --- Theme Styles ---
    const containerBg = isIlluminateEnabled ? "bg-gray-100" : isBlackoutEnabled ? "bg-black" : "bg-gray-900"; // Used for the overall container
    ...
    const iconColor = isIlluminateEnabled ? "text-gray-500" : "text-gray-400"; // Defined but seems unused directly here, maybe inherited
    ...
    // Render individual note panel - Ensure it fills height and scrolls internally
    const renderNote = (note: Note | null) => {
        ...
        return (
            // This div takes the height given by SplitPane and allows internal scrolling
            <div className={`h-full flex flex-col ${notePanelBg} rounded-lg overflow-hidden shadow-sm`}>
                ...
                <div className={`p-3 border-b ${borderColor} flex items-center justify-between flex-shrink-0`}>
                    ...
                    <div className={`flex items-center gap-1 border rounded-full p-0.5 ${borderColor} ${isIlluminateEnabled ? 'bg-gray-50' : 'bg-gray-800/50'}`}>
                        ...
                        <button onClick={() => onTogglePublic(note.id, note.isPublic)} className={`p-1 rounded-full ${iconHoverColor} ${note.isPublic ? (isIlluminateEnabled ? 'hover:bg-cyan-100/50' : 'hover:bg-cyan-900/30') : (isIlluminateEnabled ? 'hover:bg-gray-100/50' : 'hover:bg-gray-600/30')}`} title={note.isPublic ? 'Make private' : 'Make public'}>{note.isPublic ? <Globe className={`w-3.5 h-3.5 ${isIlluminateEnabled ? 'text-cyan-600' : 'text-cyan-400'}`} /> : <Lock className="w-3.5 h-3.5" />}</button>
                ...
                <div className="flex-1 overflow-y-auto p-3 md:p-4">
                    {/* Note Content */}
                    <div className={`${proseClass} mb-4 text-sm`}>
                        <ReactMarkdown
                            remarkPlugins={[remarkMath, remarkGfm]}
                            rehypePlugins={[rehypeKatex]}
                            components={{ // Customize markdown rendering (same as before)
                                ...
                                code: ({ node, inline, className, children, ...props }) => { const match = /language-(\w+)/.exec(className || ''); return !inline ? ( <pre className={`text-[10px] leading-snug ${isIlluminateEnabled ? '!bg-gray-100 !text-gray-800' : '!bg-gray-900 !text-gray-300'} p-1.5 rounded overflow-x-auto my-1`} {...props}> <code className={`language-${match?.[1] || 'plaintext'}`}>{children}</code> </pre> ) : ( <code className={`text-[10px] ${isIlluminateEnabled ? 'bg-gray-200/70 text-gray-800' : 'bg-gray-700/70 text-gray-200'} px-0.5 rounded`} {...props}>{children}</code> ); },
                            }}
                        >{note.content}</ReactMarkdown>
                    </div>
                    {/* Key Points */}
                    {note.keyPoints && note.keyPoints.length > 0 && ( <div className={`mt-4 border-t pt-3 ${borderColor}`}> <h3 className={`text-sm font-semibold mb-1.5 flex items-center gap-1 ${headingColor}`}><Sparkles className={`w-3.5 h-3.5 ${isIlluminateEnabled ? 'text-yellow-500' : 'text-yellow-400'}`} />Key Points</h3> <ul className="space-y-1 text-xs"> {note.keyPoints.map((point, index) => ( <li key={index} className={`flex items-start gap-1.5 ${textColor}`}><span className={`flex-shrink-0 mt-1 w-1 h-1 rounded-full ${isIlluminateEnabled ? 'bg-blue-500' : 'bg-blue-400'}`}></span>{point}</li> ))} </ul> </div> )}
                    {/* Study Questions */}
                    {note.questions && note.questions.length > 0 && ( <div className={`mt-4 border-t pt-3 ${borderColor}`}> <h3 className={`text-sm font-semibold mb-2 flex items-center gap-1 ${headingColor}`}><BookOpen className={`w-3.5 h-3.5 ${isIlluminateEnabled ? 'text-purple-600' : 'text-purple-400'}`} />Study Questions</h3> <div className="space-y-3"> {note.questions.map((q, index) => ( <div key={index} className={`p-2 rounded-md ${isIlluminateEnabled ? 'bg-gray-100/80 border border-gray-200/60' : 'bg-gray-700/50 border border-gray-600/40'} text-xs`}> <p className={`${textColor} mb-1.5`}><span className="font-medium">{index + 1}.</span> {q.question}</p> <div className="space-y-1"> {q.options.map((option, optIndex) => ( <div key={optIndex} className={`px-2 py-1 rounded ${optIndex === q.correctAnswer ? (isIlluminateEnabled ? 'bg-green-100 text-green-800 font-medium' : 'bg-green-500/20 text-green-300 font-medium') : (isIlluminateEnabled ? 'bg-gray-200/70 text-gray-600' : 'bg-gray-600/60 text-gray-400')}`}>{option}</div> ))} </div> {q.explanation && ( <div className={`mt-1.5 pt-1 border-t ${isIlluminateEnabled ? 'border-gray-200/80' : 'border-gray-600/50'} ${isIlluminateEnabled ? 'text-gray-600' : 'text-gray-400'}`}><span className="font-medium">Explanation: </span>{q.explanation}</div> )} </div> ))} </div> </div> )}
                </div>
            </div>
        );
    };
    ...
    return (
        // Container fills the available space within Notes.tsx's main content area
        // Uses flex layout to position header and split pane area
        <div className={`h-full flex flex-col ${containerBg}`}>
            {/* Header for Split View */}
            <div className="flex items-center justify-between mb-2 flex-shrink-0 px-2 pt-2 sm:px-0 sm:pt-0"> {/* Adjusted padding */}
            ...
            <div className="flex-1 overflow-hidden"> {/* Use flex-1 to grow */}
                <style>{`
                    .split-pane { position: relative; height: 100%; display: flex; }
                    .split-pane > .split-pane-resizer { background: ${isIlluminateEnabled ? '#e5e7eb' : '#374151'}; width: 6px; cursor: col-resize; margin: 0 2px; border-radius: 3px; transition: background 0.2s ease; z-index: 1; }
                    .split-pane > .split-pane-resizer:hover { background: ${isIlluminateEnabled ? '#d1d5db' : '#4b5563'}; }
                    /* The direct children (panes) of SplitPane need height 100% for internal scrolling */
                    .split-pane > div { height: 100%; overflow: hidden; } /* Let renderNote handle internal scroll */
                `}</style>
                <SplitPane
                    split="vertical"
                    sizes={sizes}
                    onChange={setSizes}
                >
                    {/* Panes now have height handled by the style tag */}
