...
import {
    ...
    Lock,
    Trash2,
    Copy, // Copy still seems unused, but kept import
    Sparkles,
    BookOpen,
    X,
    SplitSquareVertical // <-- Import added here
} from 'lucide-react'; // Added SplitSquareVertical
                                    ...
                                    <div key={index} className={`p-2 rounded-md ${isIlluminateEnabled ? 'bg-gray-100/80 border border-gray-200/60' : 'bg-gray-700/50 border border-gray-600/40'} text-xs`}>
                                        ...
                                        <div className="space-y-1">
                                            {q.options.map((option, optIndex) => (
                                                <div
                                                    key={optIndex}
                                                    className={`px-2 py-1 rounded ${optIndex === q.correctAnswer
        ...
        <div className={`absolute inset-0 z-20 ${containerBg} p-2 sm:p-3 md:p-4 flex flex-col`}>
            ...
            <div className="flex items-center justify-between mb-2 flex-shrink-0">
                <h2 className={`text-lg font-semibold ${headingColor} flex items-center gap-2`}>
                    <SplitSquareVertical className="w-5 h-5" /> {/* Icon usage */}
                    Split View
                </h2>
                <button
                    onClick={onClose}
            ...
            <div className="flex-1 overflow-hidden">
                ...
                <style>{`
                    ...
                    .split-pane > .split-pane-resizer { background: ${isIlluminateEnabled ? '#e5e7eb' : '#374151'}; width: 6px; cursor: col-resize; margin: 0 2px; border-radius: 3px; transition: background 0.2s ease; z-index: 1; } /* Added z-index */
                    .split-pane > .split-pane-resizer:hover { background: ${isIlluminateEnabled ? '#d1d5db' : '#4b5563'}; }
                    .split-pane > div { overflow: hidden; height: 100%; } /* Changed overflow to hidden, applied scrolling inside renderNote */
                `}</style>
                <SplitPane
                    split="vertical"
                    sizes={sizes}
