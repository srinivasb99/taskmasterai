...
import {
  FileText,
  Upload, // Keep imports as they are, even if unused in this snippet
  Youtube,
  Mic,
  Plus,
  Search,
  ...
  X,
  ChevronRight,
  ChevronLeft, // Now used for mobile "back"
  Bot,
  FileQuestion,
  BookOpen,
  Sparkles,
  ...
  RefreshCw,
  SplitSquareVertical,
  Menu, // Could use for mobile toggle instead of List
  List, // Used for mobile toggle icon
  Briefcase,
} from 'lucide-react';
import { Sidebar } from './Sidebar'; // Assuming Sidebar is the main left nav
...
// Types (assuming interface Note is defined as before)
interface Note {
  id: string;
  title: string;
  content: string;
...
export function Notes() {
  ...
  const [isIlluminateEnabled, setIsIlluminateEnabled] = useState(() => {
    ...
    return stored ? JSON.parse(stored) : true; // Default light
  });
  ...
  // Mobile state
  const [showNotesListOnMobile, setShowNotesListOnMobile] = useState(true); // Start showing list on mobile
  ...
  // --- Effects ---
   ...
   useEffect(() => {
     const handleResize = () => {
       ...
       if (!mobile) {
         ...
         // On mobile, if a note or split view is active, hide the list initially
   ...
   }, [selectedNote, showSplitView, splitViewNotes.left, splitViewNotes.right]); // Re-evaluate on selection change too
  ...
  useEffect(() => {
    ...
    const unsubscribe = auth.onAuthStateChanged((firebaseUser) => {
      if (firebaseUser) {
        ...
        // Don't setLoading(false) here, wait for notes to load too
      } else {
        setUser(null);
        setNotes([]);
        ...
        navigate('/login');
        setLoading(false); // Set loading false if redirecting
      }
    });
    return () => unsubscribe();
  }, [navigate]);
  ...
  useEffect(() => {
    if (!user?.uid) {
      setNotes([]);
      setLoading(false); // Ensure loading stops if user logs out before listener setup
      return;
    }

    // Keep loading true until notes are fetched
    // setLoading(true); // Removed: setLoading handled in auth effect or before listener
    const q = query(
      collection(db, 'notes'),
    ...
    const unsubscribe = onSnapshot(
      ...
      (snapshot) => {
        ...
        setNotes(notesList);
        setLoading(false); // Set loading false after notes are fetched
      },
      (error) => {
  ...
  const handleSplitViewSelect = (note: Note) => {
    if (!splitViewNotes.left) {
      ...
      // Split view component will render automatically when both are set
      if (isMobile) {
          setShowNotesListOnMobile(false); // Hide list after second selection on mobile
      }
    }
    // Keep list visible on mobile until second note is selected
  };
  ...
  const closeSplitView = () => {
    setShowSplitView(false);
    setSplitViewNotes({ left: null, right: null });
    // Select the first note or show placeholder
  ...
  const handleDeleteNote = async (noteId: string) => {
    ...
    try {
      ...
      if (splitViewAltered) {
          ...
          // Decide if split view should close or stay open showing selection prompt
          if (!updatedSplitNotes.left || !updatedSplitNotes.right) {
              // Keep split view active, showing the selection prompt
              setShowSplitView(true);
               if (isMobile) {
  ...
  const handleCreateAINote = async (text: string) => {
    ...
    try {
      ...
      setUploadProgress({ progress: 0, status: '', error: error instanceof Error ? error.message : 'Failed to create AI note'});
    }
  };
  ...
  const handlePDFUpload = async (file: File) => {
    ...
    try {
      ...
      await saveNote({
        ...
        sourceUrl: processedPDF.sourceUrl || file.name,
        userId: user.uid,
        isPublic: false,
        tags: ['pdf', file.name.split('.').pop() || 'file']
      });
      ...
      setUploadProgress({ progress: 0, status: '', error: error instanceof Error ? error.message : 'Failed to process PDF'});
    }
  };
  ...
  const handleYoutubeLink = async (url: string) => {
    ...
    try {
      ...
      setUploadProgress({ progress: 0, status: '', error: error instanceof Error ? error.message : 'Failed to process YouTube video'});
    }
  };
  ...
  const notesListBg = isIlluminateEnabled // Changed variable name
  ...
  const listItemSelectedBg = isIlluminateEnabled ? "bg-blue-50 border-l-2 border-blue-500" : "bg-gray-700 border-l-2 border-blue-500"; // Added left border for selection
  const listItemBaseClass = isIlluminateEnabled ? "border-l-2 border-transparent" : "border-l-2 border-transparent"; // Base class for border transition
  ...
  return (
    <div className={`flex h-screen overflow-hidden ${containerClass} font-sans`}>
      {/* Main App Sidebar (Still on Left) */}
      <Sidebar
      ...
      <main
        className={`flex-1 flex overflow-hidden transition-all duration-300 ${
          isSidebarCollapsed ? 'ml-16 md:ml-20' : 'ml-64' // Adjust based on main Sidebar state
        }`}
      >
        {/* === 1. Main Content Area (Left/Center Panel) === */}
        <div ref={contentRef} className={`flex-1 overflow-y-auto ${mainContentBg} relative`}>
          {/* Mobile Header for Note View (when notes list is hidden) */}
          {isMobile && !showNotesListOnMobile && (
            <div className={`sticky top-0 z-10 p-2 border-b ${borderColor} ${isIlluminateEnabled ? 'bg-white/80 backdrop-blur-sm' : 'bg-gray-900/80 backdrop-blur-sm'} flex items-center justify-between`}>
              {/* Button to Show Notes List (Now on Right) */}
              <button
              ...
              {/* Action Buttons (Edit/Save) for Mobile Header */}
              <div className="w-8 h-8 flex items-center justify-center"> {/* Fixed size placeholder/container */}
          ...
          {/* Render SplitView directly if active */}
          {showSplitView && splitViewNotes.left && splitViewNotes.right ? (
            ...
            /* Render Padded Container for Single Note, Edit, Placeholder, or Split Select */
            <div className="p-4 md:p-6 lg:p-8 h-full"> {/* Ensure padding div takes height */}
              {showSplitView && (!splitViewNotes.left || !splitViewNotes.right) ? (
                  ...
                  <div className={`${noteViewBg} rounded-lg p-4 md:p-6`}>
                    <div className="space-y-3">
                      ...
                      <div className="flex flex-wrap items-center gap-2">
                        {editTags.map((tag) => (
                          <span key={tag} className={`flex items-center gap-1 px-2 py-0.5 rounded-full text-xs ${tagColors.custom} ${tagBaseBg} ${tagTextBase}`}>
                            {tag}
                            <button onClick={() => handleRemoveTag(tag)} className={`${iconHoverColor} rounded-full hover:bg-black/10 dark:hover:bg-white/10 p-0.5`} title={`Remove tag "${tag}"`}>
                        ...
                        {editTags.length < 5 && (
                          <div className="flex items-center gap-1">
                            ...
                            />
                            <button onClick={handleAddTag} className={`${buttonSecondaryClass} px-2 py-0.5 rounded-full text-xs`}>Add</button>
                          </div>
                        )}
                      </div>
                      {/* Content Textarea */}
                      ...
                      <div className="flex justify-end gap-2 pt-2">
                        ...
                        <button onClick={handleSaveEdit} disabled={isSaving || !editTitle.trim() || !editContent.trim()} className={`${buttonPrimaryClass} px-4 py-1.5 rounded-md text-sm flex items-center gap-1.5 ${isSaving ? buttonDisabledClass : ''}`}>
                  ...
                  <div className={`${noteViewBg} rounded-lg p-4 md:p-6 lg:p-8 animate-fadeIn`}>
                      ...
                      {/* Action Buttons Group */}
                       <div className={`flex items-center gap-1 flex-shrink-0 border rounded-full p-0.5 ${borderColor} ${isIlluminateEnabled ? 'bg-gray-50' : 'bg-gray-800/50'}`}>
                         ...
                         <button onClick={() => handleTogglePublic(selectedNote.id, selectedNote.isPublic)} className={`p-1.5 rounded-full ${iconHoverColor} ${selectedNote.isPublic ? (isIlluminateEnabled ? 'hover:bg-cyan-100/50' : 'hover:bg-cyan-900/30') : (isIlluminateEnabled ? 'hover:bg-gray-100/50' : 'hover:bg-gray-600/30')}`} title={selectedNote.isPublic ? 'Make private' : 'Make public'}>
                           {selectedNote.isPublic ? <Globe className={`w-4 h-4 ${isIlluminateEnabled ? 'text-cyan-600' : 'text-cyan-400'}`} /> : <Lock className="w-4 h-4" />}
                         </button>
                    ...
                    {/* Note Content */}
                     <div className={`${proseClass} mb-6`}>
                         <ReactMarkdown
                           ...
                           components={{ // Custom markdown rendering components (as before)
                    ...
                    {selectedNote.keyPoints && selectedNote.keyPoints.length > 0 && (
                      <div className={`mt-6 border-t pt-4 ${borderColor}`}>
                        <h3 className={`text-base font-semibold mb-3 flex items-center gap-1.5 ${headingClass}`}><Sparkles className={`w-4 h-4 ${isIlluminateEnabled ? 'text-yellow-500' : 'text-yellow-400'}`} />Key Points</h3>
                        <ul className="space-y-1.5 text-sm">
                          {selectedNote.keyPoints.map((point, index) => (<li key={index} className={`flex items-start gap-2 ${textColor}`}><span className={`flex-shrink-0 mt-1 w-1.5 h-1.5 rounded-full ${isIlluminateEnabled ? 'bg-blue-500' : 'bg-blue-400'}`}></span>{point}</li>))}
                        </ul>
                      </div>
                    )}
                    ...
                    {selectedNote.questions && selectedNote.questions.length > 0 && (
                      <div className={`mt-6 border-t pt-4 ${borderColor}`}>
                        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-3 gap-2">
                          <h3 className={`text-base font-semibold flex items-center gap-1.5 ${headingClass}`}><BookOpen className={`w-4 h-4 ${isIlluminateEnabled ? 'text-purple-600' : 'text-purple-400'}`} />Study Questions</h3>
                          <button onClick={handleRegenerateQuestions} disabled={isRegeneratingQuestions} className={`${buttonSecondaryClass} px-3 py-1 rounded-md text-xs flex items-center gap-1 ${isRegeneratingQuestions ? buttonDisabledClass : ''}`}>
                            {isRegeneratingQuestions ? (<> <Loader2 className="w-3 h-3 animate-spin" /> Regenerating... </>) : (<> <RefreshCw className="w-3 h-3" /> Regenerate </>)}
                          </button>
                        </div>
                        <div className="space-y-4">
                          {selectedNote.questions.map((q, index) => {
                            const questionKey = `${selectedNote?.id}-${index}`;
                              ...
                              <div key={index} className={`p-3 rounded-lg ${isIlluminateEnabled ? 'bg-gray-100/80 border border-gray-200/60' : 'bg-gray-700/50 border border-gray-600/40'}`}>
                                ...
                                <div className="space-y-1.5">
                                  {q.options.map((option, optIndex) => {
                                    ...
                                    if (isAnswered) {
                                        if (isSelected) buttonClass += isCorrect ? ` ${isIlluminateEnabled ? 'bg-green-100 border-green-300 text-green-800' : 'bg-green-500/20 border-green-500 text-green-300'} border font-medium` : ` ${isIlluminateEnabled ? 'bg-red-100 border-red-300 text-red-800' : 'bg-red-500/20 border-red-500 text-red-300'} border font-medium`;
                                        else if (isCorrect) buttonClass += ` ${isIlluminateEnabled ? 'bg-green-50/50 border-green-200 text-green-700' : 'bg-green-500/10 border-green-600/50 text-green-400'} border`;
                                        else buttonClass += ` ${isIlluminateEnabled ? 'bg-gray-100 text-gray-500' : 'bg-gray-700 text-gray-400 opacity-70'}`;
                                    }
                                    return (
                                      <button key={optIndex} onClick={() => !isAnswered && handleAnswerSelect(questionKey, optIndex)} disabled={isAnswered} className={buttonClass}>
                                        <div className="flex items-center justify-between">
                                          <span>{option}</span>
                                          {isAnswered && isSelected && (isCorrect ? <Check className={`w-3.5 h-3.5 ${isIlluminateEnabled ? 'text-green-600' : 'text-green-400'}`} /> : <X className={`w-3.5 h-3.5 ${isIlluminateEnabled ? 'text-red-600' : 'text-red-400'}`} />)}
                                        </div>
                                      </button>
                                    );
                                  })}
                                </div>
                                {isAnswered && q.explanation && (<div className={`mt-2 p-2 rounded-md text-xs ${isIlluminateEnabled ? 'bg-gray-200/70 text-gray-700' : 'bg-gray-600/60 text-gray-300'}`}><span className="font-medium">Explanation: </span>{q.explanation}</div>)}
                              </div>
                            );
                          })}
                        </div>
                ...
                <div className={`flex flex-col items-center justify-center rounded-lg ${noteViewBg} h-full text-center p-6`}> {/* Use h-full here */}
                   ...
                   <button onClick={() => setShowNewNoteModal(true)} className={`${buttonPrimaryClass} px-3 py-1.5 rounded-md text-sm flex items-center gap-1.5 mb-2`}>
                     <Plus className="w-4 h-4" /> Create Note
                   </button>
                   {!isMobile && notes.length >= 2 && !showSplitView && (
                     <button onClick={startSplitView} className={`${buttonSecondaryClass} px-3 py-1.5 rounded-md text-sm flex items-center gap-1.5`}>
                       <SplitSquareVertical className="w-4 h-4" /> Compare Notes (Split View)
                     </button>
                   )}
                </div>
              )}
            </div> // End Padded Container Div
          )} {/* End Conditional Rendering for SplitView vs Padded Container */}
        ...
        <div
          className={`w-full md:w-72 lg:w-80 xl:w-96 border-l ${borderColor} flex-shrink-0 flex flex-col ${notesListBg} transition-transform duration-300 ease-in-out ${
            isMobile ? (showNotesListOnMobile ? 'translate-x-0 absolute top-0 right-0 h-full z-20 shadow-xl' : 'translate-x-full absolute top-0 right-0 h-full z-10') : 'translate-x-0 relative' // Mobile positioning & slide
          }`}
        >
          {/* Header for List */}
          <div className={`p-3 border-b ${borderColor} flex items-center justify-between flex-shrink-0`}>
            ...
            <div className="flex items-center gap-2">
                {!isMobile && notes.length >= 2 && ( // Split view button in desktop header
                    <button
                        onClick={showSplitView ? closeSplitView : startSplitView}
                        className={`${buttonSecondaryClass} p-1.5 rounded-full ${iconHoverColor}`}
                        title={showSplitView ? "Close Split View" : "Compare Notes (Split View)"}
                    >
                        <SplitSquareVertical className={`w-4 h-4 ${showSplitView ? iconActionColor: ''}`} />
                    </button>
                )}
                <button
                ...
                {isMobile && ( // Close button for mobile slide-in panel
                    <button
              ...
              {(['all', 'personal', 'pdf', 'youtube'] as const).map(type => (
                 <button
                   key={type} onClick={() => setFilterType(type)}
                    className={`px-2.5 py-1 text-[11px] rounded-full transition-colors whitespace-nowrap ${filterType === type ? `${buttonPrimaryClass} shadow-sm` : `${buttonSecondaryClass}`}`}
                 >
          ...
          <div className="flex-1 overflow-y-auto">
            {loading && notes.length === 0 ? ( // Show loader only if notes haven't loaded yet
              <div className="flex justify-center items-center h-full p-6">
                <Loader2 className={`w-6 h-6 animate-spin ${iconColor}`} />
              </div>
            ) : !loading && notes.length === 0 ? ( // Show empty state only after loading finished
              <div className="flex flex-col items-center justify-center h-full text-center p-6">
                <FileQuestion className={`w-12 h-12 ${iconColor} mb-3`} />
                <p className={`${subheadingClass} mb-4 text-sm`}>No notes yet.</p>
                <button onClick={() => setShowNewNoteModal(true)} className={`${buttonPrimaryClass} px-3 py-1.5 rounded-md text-sm flex items-center gap-1.5`}>
                  <Plus className="w-4 h-4" /> Create Note
                </button>
              </div>
            ) : (
              <div className={`divide-y ${divideColor}`}>
                {notes
                  ...
                  .map((note) => {
                    const isNoteSelected = (!showSplitView && selectedNote?.id === note.id);
                    const isSplitLeft = showSplitView && splitViewNotes.left?.id === note.id;
                    const isSplitRight = showSplitView && splitViewNotes.right?.id === note.id;
                    const isSelectedInAnyView = isNoteSelected || isSplitLeft || isSplitRight;
                    ...
                    return (
                        <div
                        key={note.id}
                         className={`p-3 cursor-pointer transition-colors duration-150 ${listItemHoverBg} ${listItemBaseClass} ${isSelectedInAnyView ? listItemSelectedBg : ''}`}
                        onClick={() => handleSelectNote(note)}
                        >
                            <div className="flex justify-between items-start gap-2">
                                <h3 className={`text-sm font-medium mb-0.5 line-clamp-1 ${headingClass}`}>{note.title || "Untitled Note"}</h3>
                                {/* Split View Selection Indicator */}
                                {isSplitLeft && <span className={`px-1.5 py-0.5 text-[9px] rounded font-medium flex-shrink-0 bg-blue-500/20 text-blue-300`}>Left</span>}
                                {isSplitRight && <span className={`px-1.5 py-0.5 text-[9px] rounded font-medium flex-shrink-0 bg-purple-500/20 text-purple-300`}>Right</span>}
                            </div>
                            <p className={`${subheadingClass} text-xs line-clamp-2 mb-1.5`}>{note.content.substring(0, 100)}</p>
                            {/* Tags and Type */}
                            <div className="flex flex-wrap items-center gap-1 text-[10px]">
                                ...
                                {note.isPublic && <span className={`flex items-center gap-1 px-1.5 py-0.5 rounded ${tagColors.public} ${tagBaseBg} ${tagTextBase}`}><Globe className="w-3 h-3"/> Public</span>}
                                {note.tags?.slice(0, 2).map((tag) => <span key={tag} className={`px-1.5 py-0.5 rounded ${tagColors.custom} ${tagBaseBg} ${tagTextBase} truncate max-w-[60px]`} title={tag}>#{tag}</span>)}
                                {note.tags && note.tags.length > 2 && <span className={`px-1.5 py-0.5 rounded ${tagColors.custom} ${tagBaseBg} ${tagTextBase}`}>+{note.tags.length - 2}</span>}
                                <span className={`ml-auto text-gray-500 dark:text-gray-500`}>{note.updatedAt?.toDate().toLocaleDateString(undefined, { month: 'short', day: 'numeric' })}</span>
                            </div>
                        </div>
                    );
                  })}
      ...
      {showChatModal && chatNote && user && (
        <NoteChat
          note={chatNote}
          onClose={() => { setShowChatModal(false); setChatNote(null); }}
      ...
      {/* Mobile Notes List Toggle Button (Floating Action Button) */}
      {isMobile && showNotesListOnMobile === false && (
        <button
           onClick={() => setShowNotesListOnMobile(true)}
           className={`fixed bottom-4 right-4 z-30 p-3 rounded-full shadow-lg transition-all duration-300 ${buttonPrimaryClass} transform hover:scale-110 active:scale-100`}
           title="Show Notes"
        >
       ...
       {/* Global Upload/Progress Indicator */}
        {uploadProgress.status && (
            <div className={`fixed bottom-4 left-4 z-50 p-3 rounded-lg shadow-lg text-xs font-medium transition-opacity duration-300 ${isIlluminateEnabled ? 'bg-white border border-gray-200 text-gray-800' : 'bg-gray-800 border border-gray-700 text-gray-200'} ${uploadProgress.error ? (isIlluminateEnabled ? '!bg-red-100 !border-red-300 !text-red-700' : '!bg-red-900/50 !border-red-700 !text-red-300') : (uploadProgress.progress === 100 ? (isIlluminateEnabled ? '!bg-green-100 !border-green-300 !text-green-700' : '!bg-green-900/50 !border-green-700 !text-green-300') : '')}`}>
                <div className="flex items-center gap-2">
                    {uploadProgress.error ? <AlertTriangle className="w-4 h-4 text-red-500" /> : uploadProgress.progress === 100 ? <Check className="w-4 h-4 text-green-500" /> : <Loader2 className="w-4 h-4 animate-spin text-blue-500" />}
                    <span>{uploadProgress.error || uploadProgress.status}</span>
                    {uploadProgress.progress > 0 && uploadProgress.progress < 100 && !uploadProgress.error && (<span className="text-gray-500">({uploadProgress.progress}%)</span>)}
                </div>
