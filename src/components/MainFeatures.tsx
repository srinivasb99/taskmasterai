...
import {
  ...
  useInView,
  AnimatePresence,
  useMotionValue, // Import the actual hook
} from "framer-motion";
import {
  LayoutDashboard,
  NotebookPen,
  ...
  CircleUserRound,
  Coins,
  FileJson, // Added for PDF notes
  Presentation, // Placeholder icon for PPTX
} from "lucide-react"; // Make sure all used icons are imported
...
const ParallaxText = ({ children, baseVelocity = 100 }) => {
  // Use the hook imported from framer-motion
  ...
  useEffect(() => {
    ...
    const update = (timestamp) => {
      ...
      const moveBy = (baseVelocity / 60) * (elapsed / 16.67) * direction;

      // Use the transform function to handle wrapping cleanly
      const currentX = baseX.get();
      let newX = currentX + moveBy;

      // Simplified wrap logic (adjust threshold as needed)
      const wrapThreshold = -1000; // Example threshold, adjust based on your text width and repetitions
      if (direction === -1 && newX < wrapThreshold) {
        newX -= wrapThreshold;
      } else if (direction === 1 && newX > 0) {
         // For rightward movement, you might need a different logic or rely on useTransform's wrapping
         // This simple logic assumes continuous leftward motion mainly
      }
  ...
  }, [baseVelocity, direction]); // baseX is stable
  ...
  return (
    <div className="overflow-hidden whitespace-nowrap flex items-center my-8 w-full">
      ...
      >
        {/* Repeat children for seamless scrolling effect - adjust repetitions based on text length and desired effect */}
      ...
      <div className="container mx-auto px-4 relative z-10">
        <div className="flex flex-col lg:flex-row items-center gap-12">
          ...
          >
            ...
            >
              Seamlessly manage tasks, set goals, and track projects with
              intelligent due dates that automatically sync to your calendar.
            </motion.p>
          ...
          >
            <GlowingBorder className="shadow-2xl shadow-indigo-500/20">
              <div className="relative bg-gray-900 rounded-xl overflow-hidden p-1">
                {" "}
                {/* Added padding */}
                <div className="absolute inset-0 bg-gradient-to-br from-indigo-900/30 to-purple-900/30 opacity-50" />{" "}
                {/* Reduced opacity */}
                <img
...
function NotesSection() {
  ...
  const handleSaveNote = () => {
    ...
    if (isEditing && selectedNote) {
      ...
      // Reselect the note to show updated content IN THE PREVIEW
      const newlyUpdatedNote = updatedNotes.find(n => n.id === selectedNote.id);
      setSelectedNote(newlyUpdatedNote || null);
    } else {
      const newNote = {
        id: Date.now(),
        title: noteTitle,
  ...
  return (
    <section
      ref={ref}
      className="relative py-32 overflow-hidden bg-gray-900/90 backdrop-blur-sm"
    >
      {" "}
      ...
      <div className="container mx-auto px-4 relative z-10">
        <div className="flex flex-col lg:flex-row-reverse items-center gap-12">
          ...
          >
            <motion.div
              variants={itemVariants}
              className="inline-flex items-center px-4 py-2 rounded-full bg-purple-500/10 border border-purple-500/20 backdrop-blur-sm mb-6"
            >
            ...
            <motion.p
              ...
              className="text-xl text-gray-300 mb-8 leading-relaxed"
            >
              Our AI-powered note-taking system generates comprehensive notes
              from various sources with smart tagging and instant search.
            </motion.p>
          ...
          <motion.div
            ...
            style={{ x: isInView ? springX : 0 }} // Apply parallax smoothly
          ...
          >
            <GlowingBorder className="shadow-2xl shadow-purple-500/20">
              <div className="bg-gray-800/80 backdrop-blur-sm p-4 sm:p-6 rounded-xl flex flex-col lg:flex-row gap-4 min-h-[500px]">
                {" "}
                    ...
                    {filteredNotes.length === 0 ? (
                      ...
                      filteredNotes.map((note) => (
                        <motion.div
                          ...
                          className={`p-3 rounded-lg transition-colors cursor-pointer border ${
                            selectedNote?.id === note.id
                              ...
                              : "bg-gray-700/40 border-transparent hover:bg-gray-600/60"
                          }`}
                        ...
                        >
                          ...
                          <p className="text-gray-300/80 text-xs mt-1 line-clamp-2">
                ...
                <div className="w-full lg:w-2/3 flex flex-col">
                  <AnimatePresence mode="wait">
                    {selectedNote ? (
                      <motion.div
                        key={selectedNote.id} // Use ID as key for proper animation
                        initial={{ opacity: 0, y: 10 }}
                        animate={{ opacity: 1, y: 0 }}
                        exit={{ opacity: 0, y: -10 }}
                          ...
                          {selectedNote.tags.length > 0 && (
                            <div className="flex items-center gap-1">
                              ...
                              {selectedNote.tags.map((tag) => (
                                <span
                                  key={tag}
                                  className="bg-gray-600/50 px-1.5 py-0.5 rounded text-gray-300"
                                >
                                  {tag}
                                </span>
                              ))}
      ...
      <AnimatePresence>
        {showNoteModal && (
          ...
          >
            {/* Increased max-width */}
            <motion.div
              className="bg-gray-800 border border-gray-700 rounded-xl p-6 max-w-lg w-full shadow-xl"
              variants={modalVariants} // Apply variants to the content div as well
            >
              ...
              <div className="space-y-5">
                ...
                <div className="flex items-center justify-between pt-2 border-t border-gray-700/50">
                  <div className="flex items-center gap-2">
                    <button
                      className="p-1.5 text-gray-400 hover:text-purple-400 rounded-full hover:bg-gray-700/50 transition-colors"
                      title="Add Tags (Coming Soon)"
                    >
                      <Tag className="w-5 h-5" />
                    </button>
                    <button
                      className="p-1.5 text-gray-400 hover:text-blue-400 rounded-full hover:bg-gray-700/50 transition-colors"
                      title="Attach File (Coming Soon)"
                    >
                      <Paperclip className="w-5 h-5" />
                    </button>
                    <button
                      className="p-1.5 text-gray-400 hover:text-yellow-400 rounded-full hover:bg-gray-700/50 transition-colors"
...
function FriendsSection() {
  ...
  const [chatMessages, setChatMessages] = useState([
    ...
    },
    {
      ...
      time: "10:26 AM",
      avatar:
        "https://www.elevenforum.com/data/attachments/82/82529-ade63e4209709292183f654907b168f5.jpg?hash=reY-Qglwkp",
    },
    {
      id: 3,
      user: "Karen",
      ...
      <div className="container mx-auto px-4 relative z-10">
        <div className="flex flex-col lg:flex-row items-center gap-12">
          ...
          >
            ...
            <motion.h2
              variants={itemVariants}
              className="text-4xl md:text-5xl font-bold mb-6 bg-gradient-to-r from-blue-400 via-indigo-400 to-purple-400 bg-clip-text text-transparent"
            >
            ...
            <motion.p
              ...
              className="text-xl text-gray-300 mb-8 leading-relaxed"
            >
              Create individual and group chats with real-time messaging, share
              files with drag-and-drop simplicity, and organize conversations
              with smart pinning.
            </motion.p>
            ...
            <motion.div variants={containerVariants} className="space-y-4 mb-8">
              ...
              >
                ...
                <p className="text-gray-300">
                  Group chats with threaded discussions
                </p>
              </motion.div>
            </motion.div>
                  ...
                  {activeTab === "chats" && (
                    <>
                      ...
                      <form
                        ...
                        className="relative mt-auto flex-shrink-0"
                      >
                        <input
                          type="text"
                          value={message}
                        ...
                        <div className="absolute right-2 top-1/2 transform -translate-y-1/2 flex items-center gap-1">
                          <button
                            type="button"
                            className="p-1.5 text-gray-400 hover:text-gray-300 rounded-full hover:bg-gray-600/50 transition-colors"
                          >
                            <Paperclip className="w-5 h-5" />
                          </button>
                          <button
                          ...
                          <button
                            type="submit"
                            className="bg-gradient-to-r from-blue-500 to-indigo-500 p-2 rounded-full disabled:opacity-50"
                        ...
                        >
                          <div className="flex items-center gap-3">
                            <div className="relative flex-shrink-0">
                              ...
                              <div
                                className={`absolute bottom-0 right-0 w-3 h-3 rounded-full border-2 border-gray-700/80 ${
                          ...
                          <div className="flex gap-1.5">
                            <button
                              className="p-2 text-blue-400 hover:text-blue-300 rounded-full hover:bg-gray-500/30 transition-colors"
                              title="Send Message"
                            >
                              <MessageSquare className="w-4 h-4" />
                            </button>
                            <button
                              className="p-2 text-gray-400 hover:text-gray-300 rounded-full hover:bg-gray-500/30 transition-colors"
                              title="More Options"
                            >
                  ...
                  {activeTab === "requests" && (
                    <div className="space-y-3 flex-grow overflow-y-auto pr-1">
                      {friendRequests.length === 0 ? (
                        <p className="text-center text-gray-400/80 py-10 text-sm">
...
function CommunitySection() {
  ...
  const [communityFiles, setCommunityFiles] = useState([
    {
      id: 1,
      fileName: "Market Research Q3.pdf",
    ...
    {
      id: 2,
      fileName: "Project Timeline V2.xlsx",
    ...
    {
      id: 3,
      fileName: "Presentation Slides - Final.pptx",
    ...
    },
    {
      ...
      uploadedAt: new Date(Date.now() - 400000000).toISOString(),
      userId: "user1",
      userName: "Michael Chen",
      userAvatar: "https://i.pravatar.cc/40?img=11",
      cost: 60,
  ...
  const [yourFiles, setYourFiles] = useState([
    {
      id: 4,
      fileName: "My Research Notes.docx",
      uploadedAt: new Date(Date.now() - 259200000).toISOString(),
      type: "docx",
    },
    {
      id: 5,
      fileName: "Budget Forecast Q1.xlsx",
  ...
  const getFileIcon = (type) => {
    switch (type?.toLowerCase()) {
      case "pdf":
        return <FileJson className="w-5 h-5 text-red-400" />;
      case "xlsx":
        return <LayoutDashboard className="w-5 h-5 text-green-400" />; // Placeholder
      case "docx":
        return <FileText className="w-5 h-5 text-blue-400" />;
      case "pptx":
        return <Presentation className="w-5 h-5 text-orange-400" />; // Placeholder, import if needed
      default:
      ...
      <div className="container mx-auto px-4 relative z-10">
        <div className="flex flex-col lg:flex-row-reverse items-center gap-12">
          ...
          >
            <motion.div
              variants={itemVariants}
              className="inline-flex items-center px-4 py-2 rounded-full bg-pink-500/10 border border-pink-500/20 backdrop-blur-sm mb-6"
            >
            ...
            <motion.h2
              variants={itemVariants}
              className="text-4xl md:text-5xl font-bold mb-6 bg-gradient-to-r from-pink-400 via-purple-400 to-indigo-400 bg-clip-text text-transparent"
            >
            ...
            <motion.p
              ...
              className="text-xl text-gray-300 mb-8 leading-relaxed"
            >
              Share and discover files, notes, and resources with fellow users.
              Build your network, learn from others, and contribute to a
              growing knowledge base using tokens.
            </motion.p>
            ...
            >
              ...
              <motion.div
                variants={itemVariants}
                className="p-4 rounded-xl bg-gradient-to-br from-purple-500/10 to-indigo-500/10 border border-gray-700/50 backdrop-blur-sm"
                whileHover={{ scale: 1.05, y: -5 }}
              >
                <p className="text-gray-300 text-sm mb-1">Shared Files</p>
                <h3 className="text-white font-bold text-2xl">
                  {communityFiles.length + yourFiles.length}
                </h3>
              </motion.div>
              <motion.div
                variants={itemVariants}
                className="p-4 rounded-xl bg-gradient-to-br from-pink-500/10 to-purple-500/10 border border-gray-700/50 backdrop-blur-sm"
                whileHover={{ scale: 1.05, y: -5 }}
              >
                ...
                <div className="flex-grow overflow-y-auto space-y-6 pr-1">
                  ...
                  <div>
                    ...
                    <div className="space-y-3">
                      {filteredCommunityFiles.length === 0 ? (
                        <p className="text-gray-400/80 text-sm text-center py-4">
                          ...
                          >
                            <div className="flex items-start gap-3">
                              ...
                              <div className="flex-grow">
                                <div className="flex items-center justify-between mb-1">
                                  <span className="font-medium text-indigo-300 text-sm truncate pr-2">
                  ...
                  <div>
                    ...
                    <div className="space-y-3">
                      {yourFiles.length === 0 ? (
                        <p className="text-gray-400/80 text-sm text-center py-4">
                          You haven't shared any files yet. Upload to earn
                          tokens!
                        </p>
                      ) : (
                        yourFiles.map((file) => (
                          ...
                          >
                            <div className="flex items-start gap-3">
                              ...
                              <div className="flex-shrink-0 flex items-center gap-1.5">
                                <button
                                  className="p-1.5 text-gray-400 hover:text-white rounded hover:bg-gray-600/50 transition-colors"
                                  title="Edit (Coming Soon)"
                                >
                                  <Edit2 className="w-3.5 h-3.5" />
                                </button>
                                <button
                                  className="p-1.5 text-gray-400 hover:text-red-400 rounded hover:bg-gray-600/50 transition-colors"
...
function AIAssistantSection() {
  ...
  const [chatHistory, setChatHistory] = useState([
    {
      role: "assistant",
      content: "Hello! I'm TaskMaster AI. How can I boost your productivity today?",
    },
  ]);
  const [isTyping, setIsTyping] = useState(false);
  ...
  const handleSendMessage = (e) => {
    ...
    setTimeout(() => {
      ...
      if (lowerCaseMsg.includes("summarize") || lowerCaseMsg.includes("summary")) {
        response =
          "Sure, please provide the text or document you want me to summarize. I can extract key points quickly.";
      } else if (
        lowerCaseMsg.includes("task") ||
        lowerCaseMsg.includes("todo") ||
        lowerCaseMsg.includes("remind me")
      ) {
        response =
          "Task management is my specialty! I can create a new task, list your upcoming tasks, or set a reminder. What would you like to do?";
      } else if (
        lowerCaseMsg.includes("schedule") ||
        lowerCaseMsg.includes("calendar") ||
        lowerCaseMsg.includes("meeting")
      ) {
        response =
          "I can help with scheduling! Tell me the event details, or I can suggest optimal times based on your calendar.";
      } else if (
        lowerCaseMsg.includes("idea") ||
        lowerCaseMsg.includes("brainstorm")
      ) {
        response =
          "Let's brainstorm! What topic are you thinking about? I can generate ideas or help structure your thoughts.";
      } else if (
        ...
        response =
          "Hi there! Ready to tackle some tasks or need assistance with something specific?";
      } else {
        response =
          "That's an interesting query! While I'm still learning, I can help with summarizing text, managing tasks, scheduling, and brainstorming. How can I assist with those?";
      }
  ...
  return (
    <section ref={ref} className="relative py-32 overflow-hidden bg-gray-900">
      ...
      <ParallaxText baseVelocity={5}>
        AI ASSISTANT PRODUCTIVITY INSIGHTS
      </ParallaxText>
      ...
      <div className="container mx-auto px-4 relative z-10">
        <div className="flex flex-col lg:flex-row items-center gap-12">
          ...
          >
            <motion.div
              variants={itemVariants}
              className="inline-flex items-center px-4 py-2 rounded-full bg-cyan-500/10 border border-cyan-500/20 backdrop-blur-sm mb-6"
            >
            ...
            <motion.h2
              variants={itemVariants}
              className="text-4xl md:text-5xl font-bold mb-6 bg-gradient-to-r from-cyan-400 via-blue-400 to-indigo-400 bg-clip-text text-transparent"
            >
            ...
            <motion.p
              ...
              className="text-xl text-gray-300 mb-8 leading-relaxed"
            >
              Get instant answers, task optimization suggestions, and time
              management help from your intelligent productivity partner.
            </motion.p>
            ...
            <motion.div variants={containerVariants} className="space-y-4 mb-8">
              ...
              >
                ...
                <p className="text-gray-300">
                  Intelligent time management assistance
                </p>
              </motion.div>
            </motion.div>
...
function CalendarSection() {
  ...
  const events = {
    ...
    12: [
      { id: 2, title: "Design Review", color: "bg-purple-500" },
      { id: 3, title: "Client Call", color: "bg-pink-500" },
    ],
      ...
      <div className="container mx-auto px-4 relative z-10">
        <div className="flex flex-col lg:flex-row-reverse items-center gap-12">
          ...
          >
            <motion.div
              variants={itemVariants}
              className="inline-flex items-center px-4 py-2 rounded-full bg-green-500/10 border border-green-500/20 backdrop-blur-sm mb-6"
            >
            ...
            <motion.h2
              variants={itemVariants}
              className="text-4xl md:text-5xl font-bold mb-6 bg-gradient-to-r from-green-400 via-teal-400 to-cyan-400 bg-clip-text text-transparent"
            >
            ...
            <motion.p
              ...
              className="text-xl text-gray-300 mb-8 leading-relaxed"
            >
              Seamlessly integrate tasks and events. Visualize your schedule with
              smart due dates and AI-powered suggestions.
            </motion.p>
            ...
            <motion.div variants={containerVariants} className="space-y-4 mb-8">
              ...
              >
                ...
                <p className="text-gray-300">
                  Recurring tasks and event reminders
                </p>
              </motion.div>
          ...
          <motion.div
            className="lg:w-1/2 w-full"
            style={{ y: springY }}
            // Apply transition directly if preferred over style prop only
                    ...
                    return (
                      <motion.div
                        ...
                        className={`aspect-square rounded-lg flex flex-col items-center justify-start p-1.5 cursor-pointer relative transition-colors duration-200
                            ${
                              isToday
                                ? "bg-gradient-to-br from-green-500/80 to-teal-500/80"
                  ...
                  <div className="space-y-2">
                    {(events[today] || []).length > 0 ? (
                      (events[today] || []).map((event) => (
                        <div
                          key={event.id}
                          className="flex items-center gap-2 p-2 rounded-md bg-gray-700/50 text-xs"
                        >
                          <div
                            className={`w-2 h-2 rounded-full flex-shrink-0 ${event.color}`}
                          ></div>
    ...
    <section ref={ref} className="relative py-32 overflow-hidden bg-gray-900">
      ...
      <div className="container mx-auto px-4 relative z-10">
        ...
        >
          ...
          >
            Hear how TaskMaster AI is transforming productivity and
            collaboration for users like you.
          </motion.p>
        </motion.div>
...
function CTASection() {
  ...
  return (
    <section ref={ref} className="relative py-32 overflow-hidden bg-gray-900">
      <FloatingElements count={20} className="opacity-60" />{" "}
      {/* Adjusted opacity */}
      <div className="container mx-auto px-4 relative z-10">
        <motion.div
        ...
        >
          ...
          >
            Join thousands of professionals elevating their productivity with
            TaskMaster AI's intelligent features. Get started today!
          </motion.p>
