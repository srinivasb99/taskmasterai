        ...
        >
          ...
          <div className="mt-auto flex flex-col gap-4">
            ...
            {!isSettingsPage && (
              ...
              >
                <Crown
                  className={`min-w-[1.25rem] transition-transform duration-200 ${isCollapsed && !isMobileMenuOpen ? "w-6 h-6" : "w-5 h-5 mr-2"}`}
                  strokeWidth={2}
                />
                <span className={`transition-opacity duration-200 ${isCollapsed && !isMobileMenuOpen ? 'opacity-0 w-0' : 'opacity-100 whitespace-nowrap'}`}>
                  Upgrade to Premium
                </span>
              </button>
            )}
