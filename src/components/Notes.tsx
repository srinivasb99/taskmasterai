...
export function Notes() {
    ...
    // Chat State
    const [isChatOverlayVisible, setIsChatOverlayVisible] = useState(false); // Used only for non-PDF overlay
    const [chatNote, setChatNote] = useState<Note | null>(null); // Tracks which note the overlay is for
    const noteChatRef = useRef<NoteChatHandle>(null); // Ref for side-by-side chat
    ...
    const [showPdfViewer, setShowPdfViewer] = useState(false); // Controls if PDF view is active
    ...
    useEffect(() => {
        ...
        return () => unsubscribe();
    }, [user, selectedNote?.id, showSplitView, splitViewNotes.left?.id, splitViewNotes.right?.id, chatNote?.id]);

    // Scroll content to top
    useEffect(() => { if (contentRef.current) { contentRef.current.scrollTop = 0; } }, [selectedNote, isEditing, splitViewNotes]); // Removed showPdfViewer dependence here

    // Reset PDF viewer state when note changes or isn't PDF
    useEffect(() => {
        if (selectedNote?.type !== 'pdf' || !selectedNote.sourceUrl) { setShowPdfViewer(false); setNumPages(null); setPdfError(null); setPdfScale(1.0); setPdfRotation(0); setCurrentPage(1); setShowHighlightButtons(false); setSelectedPdfText(null); }
        else { setShowPdfViewer(false); setNumPages(null); setPdfError(null); setPdfScale(1.0); setPdfRotation(0); setCurrentPage(1); setShowHighlightButtons(false); setSelectedPdfText(null); }
    }, [selectedNote]);

    // --- Handlers ---
    const handleToggleSidebar = () => setIsSidebarCollapsed(prev => !prev);
    const handleSelectNote = (note: Note) => { if (showSplitView) { handleSplitViewSelect(note); } else { setSelectedNote(note); setIsEditing(false); setQuestionAnswers({}); if (isMobile) { setShowNotesListOnMobile(false); } setIsChatOverlayVisible(false); setChatNote(null); } }; // Close overlay chat on note select
    const handleSplitViewSelect = (note: Note) => { if (!splitViewNotes.left) { setSplitViewNotes({ ...splitViewNotes, left: note }); } else if (!splitViewNotes.right && note.id !== splitViewNotes.left.id) { setSplitViewNotes({ ...splitViewNotes, right: note }); if (isMobile) { setShowNotesListOnMobile(false); } } setIsChatOverlayVisible(false); setChatNote(null); }; // Close overlay chat on split select
    ...
    const handleChatWithNote = (note: Note) => {
        if (note.type === 'pdf') {
             ...
             // No need to directly open chat, it appears automatically with PDF view
        } else {
             setChatNote(note); // Set note for overlay
             setIsChatOverlayVisible(true); // Show overlay
             setSelectedNote(note); // Also select the note in the main view
    ...
    // PDF Viewer Handlers
    const onDocumentLoadSuccess = ({ numPages: nextNumPages }: { numPages: number }) => { setNumPages(nextNumPages); setPdfError(null); setCurrentPage(1); };
    ...
    const handlePdfTextSelection = useCallback(() => {
        if (!pdfViewerContainerRef.current) return;
        ...
        if (selection && !selection.isCollapsed && selection.rangeCount > 0) {
            ...
            // Check if selection is meaningfully inside the PDF container
            if (selectedText.length > 2 && pdfViewerContainerRef.current.contains(range.commonAncestorContainer)) {
                setSelectedPdfText(selectedText);
                // Position button slightly above the selection end
                const top = rangeRect.top - containerRect.top + pdfViewerContainerRef.current.scrollTop - 35; // Adjust offset as needed
                const left = rangeRect.right - containerRect.left + pdfViewerContainerRef.current.scrollLeft - (rangeRect.width / 2); // Center horizontally
                setHighlightButtonPosition({ top: Math.max(0, top), left: Math.max(0, left) }); // Ensure positive coords
                setShowHighlightButtons(true);
                return; // Exit early
            }
        }
        // If no valid selection or outside container, hide buttons
        if (showHighlightButtons) {
            setShowHighlightButtons(false);
            setSelectedPdfText(null);
            setHighlightButtonPosition(null);
        }
    }, [showHighlightButtons]); // Added dependency


    // Click outside handler for highlight buttons
    useEffect(() => {
        const handleClickOutside = (event: MouseEvent) => {
            // Check if click is outside the highlight buttons (if they are shown)
            if (showHighlightButtons && highlightButtonPosition) {
                // A simple check: if the click target is not one of the buttons or their container
                // This might need refinement depending on the button container structure
    ...
    }, [showHighlightButtons, highlightButtonPosition]); // Re-run if buttons appear/disappear
    ...
    return (
        <div className={`flex h-screen overflow-hidden ${containerClass} font-sans`}>
            ...
            <main className={`flex-1 flex overflow-hidden transition-all duration-300 ${ isSidebarCollapsed ? 'ml-16 md:ml-20' : 'ml-64' }`}>
                ...
                {/* === 1. Main Content Area === */}
                <div ref={contentRef} className={`flex-1 overflow-y-auto ${mainContentBg} relative ${showSideBySideChat ? 'overflow-hidden' : ''}`}> {/* Adjust overflow for side-by-side */}
                    {/* Mobile Header */}
                    {isMobile && !showNotesListOnMobile && ( <div className={`sticky top-0 z-10 p-2 border-b ${borderColor} ${isIlluminateEnabled ? 'bg-white/80 backdrop-blur-sm' : 'bg-gray-900/80 backdrop-blur-sm'} flex items-center justify-between`}> <button onClick={() => setShowNotesListOnMobile(true)} className={`p-1.5 rounded-md ${buttonSecondaryClass} ${iconHoverColor}`} title="Show Notes"> <List className="w-4 h-4" /> </button> <span className={`text-sm font-medium truncate px-2 ${headingClass}`}> {isEditing ? 'Editing Note' : selectedNote?.title || 'Note'} </span> <div className="w-8 h-8 flex items-center justify-center"> {selectedNote && !isEditing && !showPdfViewer && ( <button onClick={handleEditNote} className={`p-1.5 rounded-md ${iconHoverColor} ${iconActionHoverBg}`} title="Edit note"> <Edit2 className="w-4 h-4" /> </button> )} {isEditing && ( <button onClick={handleSaveEdit} disabled={isSaving || !editTitle.trim() || !(editContent||'').trim()} className={`p-1.5 rounded-md ${buttonPrimaryClass} ${isSaving ? buttonDisabledClass : ''}`} title="Save Changes"> {isSaving ? <Loader2 className="w-4 h-4 animate-spin" /> : <Save className="w-4 h-4" />} </button> )} </div> </div> )}
                    ...
                    {/* --- Dynamic Content Rendering --- */}
                     {showSplitView && splitViewNotes.left && splitViewNotes.right ? (
                         ...
                         // --- Side-by-Side PDF Viewer and Chat ---
                         <div className="flex flex-col md:flex-row h-full overflow-hidden">
                             ...
                             <div className="w-full md:w-3/5 lg:w-2/3 h-1/2 md:h-full flex flex-col overflow-hidden relative border-r border-gray-200 dark:border-gray-700">
                                 ...
                                 <div className={`flex-shrink-0 p-1.5 border-b ${borderColor} ${isIlluminateEnabled ? 'bg-gray-50' : 'bg-gray-800/50'} flex items-center justify-between gap-1 sticky top-0 z-10`}>
                                     ...
                                     <div className="flex items-center gap-1">
                                         <button onClick={() => changePage(-1)} disabled={currentPage <= 1} className={`p-1.5 rounded-md ${buttonSecondaryClass} ${iconHoverColor} ${currentPage <= 1 ? 'opacity-50' : ''}`} title="Previous Page"> <ChevronLeft className="w-4 h-4" /> </button>
                                         <button onClick={() => changePage(1)} disabled={currentPage >= (numPages || 1)} className={`p-1.5 rounded-md ${buttonSecondaryClass} ${iconHoverColor} ${currentPage >= (numPages || 1) ? 'opacity-50' : ''}`} title="Next Page"> <ChevronRight className="w-4 h-4" /> </button>
                                 ...
                                 <div ref={pdfViewerContainerRef} onMouseUp={handlePdfTextSelection} className={`flex-1 overflow-auto relative ${isIlluminateEnabled ? 'bg-gray-100' : 'bg-gray-800/30'}`}>
                                     {pdfError ? ( <div className="p-6 text-center text-red-500 flex flex-col items-center justify-center h-full"> <AlertTriangle className="w-10 h-10 mb-3" /> <p className="text-sm font-medium">Error Loading PDF</p> <p className="text-xs mt-1">{pdfError}</p> </div> ) : (
                                        <Document file={selectedNote.sourceUrl} onLoadSuccess={onDocumentLoadSuccess} onLoadError={onDocumentLoadError} loading={<div className="flex justify-center items-center h-64"><Loader2 className="w-6 h-6 animate-spin text-blue-500"/></div>} className="flex flex-col items-center pdf-document pt-2 pb-4">
                                             <Page
                                                 ...
                                                 onRenderSuccess={() => { /* Potentially re-check selection */ }}
                                            />
                                        </Document>
                                     )}
                                     {/* Highlight Buttons */}
                                     {showHighlightButtons && highlightButtonPosition && (
                                         <div
                                             ...
                                             style={{ top: `${highlightButtonPosition.top}px`, left: `${highlightButtonPosition.left}px` }}
                                             onClick={(e) => e.stopPropagation()} // Prevent click bubbling to clear selection
                                        >
                                             <button onClick={handleExplainHighlight} className="text-xs text-white dark:text-black bg-blue-500 dark:bg-blue-400 hover:bg-blue-600 dark:hover:bg-blue-500 px-2 py-1 rounded">Explain</button>
                                             <button onClick={handleChatAboutHighlight} className="text-xs text-white dark:text-black bg-green-500 dark:bg-green-400 hover:bg-green-600 dark:hover:bg-green-500 px-2 py-1 rounded">Chat</button>
                                         </div>
                                     )}
                                 </div>
                             </div>
                             {/* Chat Section */}
                             <div className="w-full md:w-2/5 lg:w-1/3 h-1/2 md:h-full flex flex-col overflow-hidden">
                                 <NoteChat
                                     ref={noteChatRef}
                                     note={selectedNote}
                                     onClose={() => setShowPdfViewer(false)} // Close button hides PDF/Chat view
                         ...
                         <div className={`p-0 md:p-6 lg:p-8 h-full ${isEditing ? 'flex flex-col' : ''}`}>
                            {showSplitView && (!splitViewNotes.left || !splitViewNotes.right) ? (
                                ...
                                isEditing ? (
                                    ...
                                    <div className={`${noteViewBg} rounded-lg p-4 md:p-6 flex flex-col h-full`}>
                                        ...
                                        <MDEditor
                                            ...
                                            height="100%"
                                            preview="live" // Changed preview mode
                                            textareaProps={{
                                            placeholder: "Start writing your note in Markdown...",
                                            className: `${inputBg} ${inputTextColor} ${placeholderColor}`
                                            }}
                                            className="[&_.w-md-editor-preview]:!bg-transparent [&_.w-md-editor-input]:!bg-transparent [&_.w-md-editor-toolbar]:!bg-gray-100/50 dark:[&_.w-md-editor-toolbar]:!bg-gray-800/50" // Basic theme adjustments
                                        />
                                        </div>
                                        {/* Action Buttons */}
            ...
            {/* Overlay Chat (Only for non-PDF notes) */}
            {!showSideBySideChat && chatNote && user && geminiApiKey && (
                 <NoteChat
                     key={chatNote.id} // Ensure re-mount on note change
                     note={chatNote}
