...
// Use the same enhanced prose class as Notes.tsx for consistency but maybe smaller base size
...
const renderNote = (note: Note | null) => {
    if (!note) {
        // This case should technically not be hit if called from the main component structure
        return ( <div className={`h-full flex flex-col items-center justify-center ${notePanelBg} rounded-lg p-4 ${textColor} text-center text-sm`}> Note loading... </div> );
    }
    ...
    return (
        ...
        <div className={`h-full flex flex-col ${notePanelBg} rounded-lg overflow-hidden shadow-sm`}>
            {/* Note Header (Fixed) */}
            <div className={`p-2 md:p-3 border-b ${borderColor} flex items-center justify-between flex-shrink-0`}>
                <h2 className={`text-sm font-semibold ${headingColor} truncate pr-2`} title={note.title}>{note.title || 'Untitled'}</h2>
                <div className={`flex items-center gap-1 border rounded-full p-0.5 ${borderColor} ${isIlluminateEnabled ? 'bg-gray-50' : 'bg-gray-800/50'}`}>
                    <button onClick={() => onChat(note)} className={`p-1 rounded-full ${iconHoverColor} ${iconActionHoverBg}`} title="Chat with this note"><MessageCircle className="w-3.5 h-3.5" /></button>
                    ...
                    <button onClick={async () => {
                        ...
                        }} className={`p-1 rounded-full ${iconDeleteHoverColor} ${iconDeleteHoverBg}`} title="Delete note"><Trash2 className="w-3.5 h-3.5" /></button>
                </div>
            </div>
            ...
            <div className="flex-1 overflow-y-auto p-3 md:p-4 scrollbar-thin scrollbar-thumb-gray-400 dark:scrollbar-thumb-gray-600 scrollbar-track-transparent">
                ...
                <div className={`${proseClass} mb-4`}>
                    <ReactMarkdown remarkPlugins={[remarkMath, remarkGfm]} rehypePlugins={[rehypeKatex]} components={{ /* Reuse prose components styling */ }} >{note.content || ''}</ReactMarkdown>
                </div>
                {/* Key Points */}
...
return (
    // Use h-full and flex column layout, padding added for visual separation
    <div className={`h-full flex flex-col ${containerBg} p-2 md:p-4`}>
        ...
        <div className="flex-1 overflow-hidden">
            ...
            <style>{`
                .split-view-container { height: 100%; }
                .split-view-container > .split-pane { height: 100% !important; position: relative; }
                /* Style the resizer */
                .split-pane > .split-pane-resizer {
                    background: ${isIlluminateEnabled ? '#e5e7eb' : '#374151'}; /* gray-200 / gray-700 */
                    opacity: 1;
                    z-index: 10; /* Ensure resizer is clickable */
                    box-sizing: border-box;
                    background-clip: padding-box;
                    width: 8px; /* Slightly wider for easier grabbing */
                    margin: 0 -3px; /* Adjust margin to center the visual part */
                    border-left: 3px solid transparent;
                    border-right: 3px solid transparent;
                    cursor: col-resize;
                    transition: background-color 0.2s ease;
                }
                .split-pane > .split-pane-resizer:hover {
                    background-color: ${isIlluminateEnabled ? '#d1d5db' : '#4b5563'}; /* gray-300 / gray-600 */
                }
                /* Direct children (panes) of SplitPane */
                .split-pane > div {
                     height: 100%;
                     overflow: hidden; /* Important: Prevent pane itself from scrolling */
                     position: relative; /* Helps contain absolutely positioned elements if any */
                }
            `}</style>
            <div className="split-view-container"> {/* Wrapper div helps ensure 100% height context */}
                <SplitPane split="vertical" sizes={sizes} onChange={setSizes}>
                    {/* Panes now correctly handle height */}
                    {/* RenderNote provides the internal scrolling */}
                    <div className="h-full">{renderNote(leftNote)}</div>
                    <div className="h-full">{renderNote(rightNote)}</div>
                </SplitPane>
            </div>
        </div>
    </div>
