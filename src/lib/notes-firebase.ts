// lib/notes-firebase.ts
import { addDoc, collection, Timestamp, updateDoc, doc, deleteDoc, getDoc, setDoc } from 'firebase/firestore';
import { db } from './firebase';
import { geminiApiKey, UserTier, PREMIUM_EMAILS, PRO_EMAILS } from './dashboard-firebase'; // Import central config/types
...
// --- REMOVED UserTier, chat limits, ChatUsageData ---

// --- Helper Functions (Keep if only used here, otherwise centralize) ---
...
const GEMINI_DEFAULT_MODEL = 'gemini-1.5-flash'; // Corrected model name
...
// --- Note Usage Tracking Functions (Keep here as they are note-specific) ---
...
// --- REMOVED getUserTier, getUserChatUsage, updateUserChatUsage ---

// --- AI Processing Functions (Unchanged) ---
...
---`;
        ...
        if (qText.startsWith("Error:")) { throw new Error(`Question generation failed: ${qText}`); }

        let newQuestions: NoteData['questions'] = [];
        const blocks = qText.split(/---DIVIDER---/i);
        console.log(`Received ${blocks.length -1} potential question blocks for regeneration.`);
        ...
        for (const block of blocks) {
            ...
            if(qM && oM && cM && eM){ const qT=qM[1].trim(); const oL=[oM[1],oM[2],oM[3],oM[4]].map(o=>o.replace(/^(.*?)\s*(?:B\)|C\)|D\)|Correct:).*$/is,'$1').trim()); const cL=cM[1].toUpperCase(); const eT=eM[1].trim(); if(qT && oL.length===4 && oL.every(o=>o) && ['A','B','C','D'].includes(cL) && eT) { newQuestions.push({question:qT,options:oL,correctAnswer:['A','B','C','D'].indexOf(cL),explanation:eT}); } else { console.warn("Partial regen Q parse - Data validation failed:", {qT, oL, cL, eT}); } } else { if(T.length > 10) console.warn("Regen Q structure parse fail for block:", T.substring(0, 100) + "..."); }
        }

        if (newQuestions.length === 0) { console.error("Failed to parse ANY questions from regeneration response. Raw response was:"); console.error("<<< START RAW RESPONSE >>>"); console.error(qText); console.error("<<< END RAW RESPONSE >>>"); throw new Error(`AI response format was unusable for question regeneration.`); }
        console.log(`Successfully parsed ${newQuestions.length} regenerated questions for note ${noteId}.`);
        if (newQuestions.length < 10) { console.warn(`Warning: Regenerated fewer than 10 questions (${newQuestions.length}).`) }
